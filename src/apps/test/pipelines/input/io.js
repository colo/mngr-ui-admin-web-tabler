...
const debug = Debug("apps:test:pipelines:input:io")
...
export default new Class({
  ...
  options: {
    ...
    io: {
			...
			// rooms: ['test'], //atomatically join connected sockets to this rooms
			routes: {
				// 'app.doc': [{
				// 	// path: ':param',
  ...
  //
  // },
  test: function(socket, next, doc){
    // let {type} = doc
    debug('test %o', doc)

    if(doc.status){
      debug('ERROR test %o', doc)
    }
    ...
    //   debug('test %o', doc)
    //   // Array.each(doc.test.tags, function(tag){
    //   //   debug('TAG %s', tag)
    ...
    //   // }.bind(this))
    //   //
    //   // Array.each(doc.test.hosts, function(host){
    //   //   debug('HOST %s', host)
    //   //   // this.io.emit('/tags/'+tag)
    //   //   this.io.emit('/', {
    //   //     params: { prop: 'hosts' },
  ...
  //
	// 	// arguments[1]()
	// 	// this.io.to('test').emit('response', 'a new user has joined the room saying '+arguments[2]);
	// 	// next(socket)
	// },
  ...
  initialize: function(options){
    ...
    this.add_io(IO()[options.index])
		...
		this.profile('test_init');//test profiling
    ...
    this.addEvent('onExit', function(){
      ...
      // if(this.io.disconnected == false)
		...
		this.log('test', 'info', 'test tested');
  },
