...
const debug = Debug('apps:hosts:pipelines:periodical')
...
Array.each(IO(), function (io, index) {
  ios.push({
    id: 'input.hosts.periodical' + index,
    module: InputIO,
    index: index
  },)
})
...
export default {
  ...
  filters: [
    ...
    // function (doc, opts, next, pipeline) {
    //   debug('MERGE %o %o', doc, buffer, pipeline.get_input_by_id('input.hosts.periodical').conn_pollers)
    //
    //   let timeout
    //
    //   let _merge = function () {
    ...
    //
    //   if (buffer.length === pipeline.get_input_by_id('input.hosts.periodical').conn_pollers) { // || buffer_expire < Date.now()
    //     _merge()
    //   } else {
    //     buffer.push(doc)
  ...
  output: [
    ...
    //
    //   if (!payload.err) { EventBus.$emit('input.hosts.periodical.' + payload.metadata.input, payload) }
    //
