...
const debug = Debug('apps:logs:web:libs:historical')
...
export default function (data, metadata, key, vm) {
  // debug('HISTORICAL WEB CALLBACK data %s %o', key, data, metadata, vm)

  if (data['logs_historical_' + vm.period]) {
    let _data
    if (data['logs_historical_' + vm.period]) _data = data['logs_historical_' + vm.period] // comes from 'Range'
    ...
    **/
    ...
    let _tmp_historical_world_map_city_counter = {}
    let _tmp_historical_world_map_country_counter = {}

    let geoip = []

    debug('HISTORICAL WEB CALLBACK data %s %o', key, _data, metadata, vm.period)
    Array.each(_data, function (row) {
      ...
      if (/nginx|apache/.test(path)) { //  && start >= smallest_start discard any document that is previous to our smallest_start timestamp
        ...
        * Traffic
        **/
        let hosts = (Array.isArray(host)) ? host : [host]
        let domains = (Array.isArray(domain)) ? domain : [domain]
        ...
        **/
        if (row.data.user_agent) {
          Object.each(row.data.user_agent, function (user_agent, user_agent_prop) {
            Object.each(user_agent, function (value, prop) {
              if (/.*\.detailed$/.test(user_agent_prop)) {
                if (!user_agent_counters[user_agent_prop]) user_agent_counters[user_agent_prop] = {}
                if (!user_agent_counters[user_agent_prop][prop]) user_agent_counters[user_agent_prop][prop] = 0
                value = value * 1
                if (!isNaN(value)) {
                  user_agent_counters[user_agent_prop][prop] += value
                }
              }
            })
          })
        ...
        // debug('user_agent_counters', domains_counter, hosts_counter, user_agent_counters)
        // /**
        // * Traffic - status
        // **/
        // if (row.data.status) {
        ...
        //   // debug('HISTORICAL HOST CALLBACK referer %o', row.data.referer.referer, row.data.referer.medium)
    ...
    /**
    * TEMP searching Android < 7.1.1 vs Total
    **/
    let user_agent_counters_totals = {}

    Object.each(user_agent_counters, function (counter, counter_prop) {
      Object.each(counter, function (value, prop) {
        if (!user_agent_counters_totals[counter_prop]) user_agent_counters_totals[counter_prop] = 0
        user_agent_counters_totals[counter_prop] += value
      })
    })

    let user_agent_counters_android = 0
    let user_agent_counters_android7 = 0

    Object.each(user_agent_counters['os.detailed'], function (value, os) {
      if (/^Android/.test(os)) {
        user_agent_counters_android += value
        let version = /^Android\s(.*)/.exec(os)
        if (version !== null && version[1]) {
          debug('android version', version)
          version = version[1].trim()
          let major = version.split('.')[0] * 1
          let minor = version.split('.')[1] * 1
          let minorPatch = version.split('.')[2] * 1
          if (!isNaN(major) && major <= 7) {
            if (major === 7) {
              if (!isNaN(minor) && minor <= 1) {
                if (!isNaN(minorPatch) && minorPatch < 1) {
                  user_agent_counters_android7 += value
                  debug('android version < 7.1.1 %d major %d minor %d patch', major, minor, minorPatch)
                }
              }
            } else {
              user_agent_counters_android7 += value
          ...
          //   let major = /^(.*)\..*$/.exec(version)
          //   if (major !== null && major[1]) {
          //     major = major[1].trim()
          //     debug('android version major', version, major)
          //   }
          //   // let minor = /^7\.(.*)/.exec(version)
          //   // if (minor !== null && minor[1]) {
          //   //   minor = minor[1].trim()
          //   //   debug('android version minor', version, minor)
          //   // }
          // } else {
          //   version *= 1
          //   if (version < 7) {
          //     user_agent_counters_android7 += value
          //     debug('android version < 7', version)
          //   }
          // }
        }
      }
    ...
    let user_agent_counters_webview = 0

    Object.each(user_agent_counters['ua.detailed'], function (value, ua) {
      if (/^Chrome\sMobile\sWebview/.test(ua)) {
        debug('UA', ua)
        user_agent_counters_webview += value
      }
    })

    debug('user_agent_counters Agents %o - Total reqs %d - Total Android %d - Android < 7.1.1 %d - Webview (App) %d',
      user_agent_counters,
      user_agent_counters_totals['os.detailed'],
      user_agent_counters_android,
      user_agent_counters_android7,
      user_agent_counters_webview
    )
    // /**
    // * Traffic
    // **/
    ...
    // let historical_hosts_counter_props = {}
    ...
    //   historical_hosts_counter_props = Object.merge(historical_hosts_counter_props, value)
    //   top_hosts_counter.push({timestamp: ts, value: value})
    // })
    //
    // debug('HISTORICAL WEB HOSTS', hosts_counter_ts, _top_hosts_counter)
    //
    // /**
    // * add missing properties with 0 value
    // **/
    // Object.each(historical_hosts_counter_props, function (val, prop) {
    ...
    // let historical_domains_counter_props = {}
    ...
    //   historical_domains_counter_props = Object.merge(historical_domains_counter_props, value)
    ...
    // * add missing properties with 0 value
    // **/
    // Object.each(historical_domains_counter_props, function (val, prop) {
    //   Array.each(top_domains_counter, function (row) {
    //     if (!row.value[prop]) row.value[prop] = 0
    //   })
    // })
    //
    // debug('HISTORICAL WEB DOMAINS', domains_counter_ts, top_domains_counter)
    // /**
    // * Traffic - status
    // **/
    // let historical_status_counter = []
    // let historical_status_counter_props = {}
    // Object.each(status_counter, function (val, ts) {
    //   if (ts < smallest_start) {
    //     delete status_counter[ts]
    //   } else {
    //     // Object.each(val, function (data, status) {
    //     //   if (!historical_status_counter[status]) historical_status_counter[status] = 0
    //     //   historical_status_counter[status] += data
    //     // })
    //     historical_status_counter_props = Object.merge(historical_status_counter_props, val)
    //
    //     historical_status_counter.push({timestamp: ts, value: val})
    //   }
    // })
    //
    // /**
    // * add missing properties with 0 value
    // **/
    // Object.each(historical_status_counter_props, function (val, prop) {
    //   Array.each(historical_status_counter, function (row) {
    //     if (!row.value[prop]) row.value[prop] = 0
    //   })
    // })
    //
    ...
    // * Traffic - method
    // **/
    // let historical_methods_counter = []
    // let historical_methods_counter_props = {}
    // Object.each(methods_counter, function (val, ts) {
    //   if (ts < smallest_start) {
    //     delete methods_counter[ts]
    //   } else {
    //     // Object.each(val, function (data, method) {
    //     //   if (!historical_methods_counter[method]) historical_methods_counter[method] = 0
    //     //   historical_methods_counter[method] += data
    //     // })
    //     historical_methods_counter_props = Object.merge(historical_methods_counter_props, val)
    //
    //     historical_methods_counter.push({timestamp: ts, value: val})
    //   }
    // })
    //
    // /**
    // * add missing properties with 0 value
    // **/
    // Object.each(historical_methods_counter_props, function (val, prop) {
    //   Array.each(historical_methods_counter, function (row) {
    //     if (!row.value[prop]) row.value[prop] = 0
    //   })
    // })
    //
    ...
    // * Traffic - pathname
    // **/
    // // let historical_pathnames_counter = []
    // // let historical_pathnames_counter_props = {}
    // // Object.each(pathnames_counter, function (val, ts) {
    // //   if (ts < smallest_start) {
    // //     delete pathnames_counter[ts]
    // //   } else {
    // //     // Object.each(val, function (data, pathname) {
    // //     //   if (!historical_pathnames_counter[pathname]) historical_pathnames_counter[pathname] = 0
    // //     //   historical_pathnames_counter[pathname] += data
    // //     // })
    // //     historical_pathnames_counter_props = Object.merge(historical_pathnames_counter_props, val)
    // //
    // //     historical_pathnames_counter.push({timestamp: ts, value: val})
    // //   }
    // // })
    // /**
    // * add missing properties with 0 value
    // **/
    // // Object.each(historical_pathnames_counter_props, function (val, prop) {
    // //   Array.each(historical_pathnames_counter, function (row) {
    // //     if (!row.value[prop]) row.value[prop] = 0
    // //   })
    // // })
    // //
    ...
    //   historical_pathnames_counter_props = Object.merge(historical_pathnames_counter_props, value)
    ...
    // })
    //
    // // debug('HISTORICAL WEB PATHNAMES', pathnames_counter, _top_pathnames_counter, top_pathnames_counter_by_ts, top_pathnames_counter)
    // /**
    // * Traffic - bytes & requests
    // **/
    // let historical_bytes_counter = []
    // let historical_requests_counter = []
    ...
    //     // historical_bytes_counter += val
    //     // historical_requests_counter += hit
    //     historical_bytes_counter.push({timestamp: ts, value: { bytes: val} })
    //     historical_requests_counter.push({timestamp: ts, value: { hits: hit } })
    //   }
    // })
    //
    // /**
    // * Traffic - type (static | dynamic)
    // **/
    // let historical_type_counter = []
    // let historical_type_counter_props = {}
    // Object.each(type_counter, function (val, ts) {
    //   if (ts < smallest_start) {
    //     delete type_counter[ts]
    //   } else {
    //     // Object.each(val, function (data, type) {
    //     //   if (!historical_type_counter[type]) historical_type_counter[type] = 0
    //     //   historical_type_counter[type] += data
    //     // })
    //     historical_type_counter_props = Object.merge(historical_type_counter_props, val)
    //     historical_type_counter.push({timestamp: ts, value: val})
    //   }
    // })
    //
    // /**
    // * add missing properties with 0 value
    // **/
    // Object.each(historical_type_counter_props, function (val, prop) {
    //   Array.each(historical_type_counter, function (row) {
    //     if (!row.value[prop]) row.value[prop] = 0
    //   })
    // })
    //
    ...
    // **/
    //
    // // let historical_addr_counter = []
    // // Object.each(addr_counter, function (val, ts) {
    // //   if (ts < smallest_start) {
    // //     delete addr_counter[ts]
    // //   } else {
    // //     // Object.each(val, function (data, addr) {
    // //     //   // if (!historical_addr_counter[addr]) historical_addr_counter[addr] = 0
    // //     //   // historical_addr_counter[addr] += data
    // //     //   let value = {}
    // //     //   value[addr] = data
    // //     //   historical_addr_counter.push({timestamp: ts, value: value })
    // //     // })
    // //     historical_addr_counter.push({timestamp: ts, value: val })
    // //   }
    // // })
    ...
    // }
    //
    // debug('HISTORICAL WEB ADDR', addr_counter, _top_addr_counter, top_addr_counter_by_ts)
    ...
    // * Traffic - user
    // **/
    // let historical_user_counter = []
    // let historical_user_counter_props = {}
    // Object.each(user_counter, function (val, ts) {
    //   if (ts < smallest_start) {
    //     delete user_counter[ts]
    //   } else {
    //     // Object.each(val, function (data, user) {
    //     //   if (!historical_user_counter[user]) historical_user_counter[user] = 0
    //     //   historical_user_counter[user] += data
    //     //
    //     // })
    //     historical_user_counter_props = Object.merge(historical_user_counter_props, val)
    //     historical_user_counter.push({timestamp: ts, value: val})
    //   }
    // })
    //
    // /**
    // * add missing properties with 0 value
    // **/
    // Object.each(historical_user_counter_props, function (val, prop) {
    //   Array.each(historical_user_counter, function (row) {
    //     if (!row.value[prop]) row.value[prop] = 0
    //   })
    // })
    //
    ...
    // * Traffic - referer
    // **/
    // let historical_referer_counter = []
    // let historical_referer_counter_props = {}
    // Object.each(referer_counter, function (val, ts) {
    //   if (ts < smallest_start) {
    //     delete referer_counter[ts]
    //   } else {
    //     historical_referer_counter_props = Object.merge(historical_referer_counter_props, val)
    //     // Object.each(val, function (data, referer) {
    //     //   if (!historical_referer_counter[referer]) historical_referer_counter[referer] = 0
    //     //   historical_referer_counter[referer] += data
    //     //
    //     // })
    //     historical_referer_counter.push({timestamp: ts, value: val})
    //   }
    // })
    //
    // /**
    // * add missing properties with 0 value
    // **/
    // Object.each(historical_referer_counter_props, function (val, prop) {
    //   Array.each(historical_referer_counter, function (row) {
    //     if (!row.value[prop]) row.value[prop] = 0
    //   })
    // })
    //
    ...
    // * Traffic - user agent
    // **/
    // let historical_user_agent_os_counter = []
    // let historical_user_agent_os_counter_props = {}
    // Object.each(user_agent_os_counter, function (val, ts) {
    //   if (ts < smallest_start) {
    //     delete user_agent_os_counter[ts]
    //   } else {
    //     // Object.each(val, function (data, os) {
    //     //   if (!historical_user_agent_os_counter[os]) historical_user_agent_os_counter[os] = 0
    //     //   historical_user_agent_os_counter[os] += data
    //     //
    //     // })
    //     historical_user_agent_os_counter_props = Object.merge(historical_user_agent_os_counter_props, val)
    //     historical_user_agent_os_counter.push({timestamp: ts, value: val})
    //   }
    // })
    //
    // /**
    // * add missing properties with 0 value
    // **/
    // Object.each(historical_user_agent_os_counter_props, function (val, prop) {
    //   Array.each(historical_user_agent_os_counter, function (row) {
    //     if (!row.value[prop]) row.value[prop] = 0
    //   })
    // })
    //
    // debug('HISTORICAL WEB TYPE', historical_type_counter)
    //
    // let historical_user_agent_engine_counter = []
    // let historical_user_agent_engine_counter_props = {}
    ...
    //     //   if (!historical_user_agent_engine_counter[engine]) historical_user_agent_engine_counter[engine] = 0
    //     //   historical_user_agent_engine_counter[engine] += data
    //     //
    //     // })
    //
    //     historical_user_agent_engine_counter_props = Object.merge(historical_user_agent_engine_counter_props, val)
    //     historical_user_agent_engine_counter.push({timestamp: ts, value: val})
    //   }
    // })
    //
    // /**
    // * add missing properties with 0 value
    // **/
    // Object.each(historical_user_agent_engine_counter_props, function (val, prop) {
    //   Array.each(historical_user_agent_engine_counter, function (row) {
    //     if (!row.value[prop]) row.value[prop] = 0
    //   })
    // })
    //
    // let historical_user_agent_device_counter = []
    // let historical_user_agent_device_counter_props = {}
    ...
    //     //   if (!historical_user_agent_device_counter[device]) historical_user_agent_device_counter[device] = 0
    //     //   historical_user_agent_device_counter[device] += data
    //     // })
    //
    //     historical_user_agent_device_counter_props = Object.merge(historical_user_agent_device_counter_props, val)
    //     historical_user_agent_device_counter.push({timestamp: ts, value: val})
    //   }
    // })
    //
    // /**
    // * add missing properties with 0 value
    // **/
    // Object.each(historical_user_agent_device_counter_props, function (val, prop) {
    //   Array.each(historical_user_agent_device_counter, function (row) {
    //     if (!row.value[prop]) row.value[prop] = 0
    //   })
    // })
    //
    // let historical_user_agent_browser_counter = []
    // let historical_user_agent_browser_counter_props = {}
    ...
    //     //   if (!historical_user_agent_browser_counter[browser]) historical_user_agent_browser_counter[browser] = 0
    //     //   historical_user_agent_browser_counter[browser] += data
    //     //
    //     // })
    //
    //     historical_user_agent_browser_counter_props = Object.merge(historical_user_agent_browser_counter_props, val)
    //     historical_user_agent_browser_counter.push({timestamp: ts, value: val})
    //   }
    // })
    //
    // /**
    // * add missing properties with 0 value
    // **/
    // Object.each(historical_user_agent_browser_counter_props, function (val, prop) {
    //   Array.each(historical_user_agent_browser_counter, function (row) {
    //     if (!row.value[prop]) row.value[prop] = 0
    //   })
    // })
    // /**
    ...
    // * GeoIP
    // **/
    // let historical_city_counter = {}
    // let historical_country_counter = {}
    // let historical_continent_counter = {}
    //
    ...
    //       if (!historical_city_counter[city]) historical_city_counter[city] = 0
    //       historical_city_counter[city] += data
    //     })
    //
    //     Object.each(country_val, function (data, country) {
    //       if (!historical_country_counter[country]) historical_country_counter[country] = 0
    //       historical_country_counter[country] += data
    //     })
    //
    //     Object.each(continent_val, function (data, continent) {
    //       if (!historical_continent_counter[continent]) historical_continent_counter[continent] = 0
    //       historical_continent_counter[continent] += data
    //     })
    //
    ...
    //       // historical_continent_counter[continent] += data
    //
    //       if (!_tmp_historical_world_map_city_counter[world_map_city]) {
    //         _tmp_historical_world_map_city_counter[world_map_city] = data
    //       } else {
    //         _tmp_historical_world_map_city_counter[world_map_city].count += data.count
    //       }
    //     })
    //
    ...
    //       // historical_continent_counter[continent] += data
    //
    //       if (!_tmp_historical_world_map_country_counter[world_map_country]) {
    //         _tmp_historical_world_map_country_counter[world_map_country] = data
    //       } else {
    //         _tmp_historical_world_map_country_counter[world_map_country].count += data.count
    //       }
    //     })
    //   }
    // })
    //
    // let historical_world_map_city_counter = []
    //
    // let historical_top_world_map_city_counter = []
    //
    // Object.each(_tmp_historical_world_map_city_counter, function (data, world_map_city) {
    //   historical_world_map_city_counter.push({
    ...
    // })
    //
    // historical_world_map_city_counter = historical_world_map_city_counter.sort((a, b) => (a.count < b.count) ? 1 : ((b.count < a.count) ? -1 : 0))
    //
    // let _top_historical_world_map_city = (historical_world_map_city_counter.length > TOP) ? TOP : historical_world_map_city_counter.length
    //
    // for (let i = 0; i < _top_historical_world_map_city; i++) {
    //   let data = historical_world_map_city_counter[i]
    //
    //   historical_top_world_map_city_counter.push(data)
    // }
    //
    // /**
    // * COUNTRY
    // **/
    // let historical_world_map_country_counter = []
    //
    // let historical_top_world_map_country_counter = []
    //
    // Object.each(_tmp_historical_world_map_country_counter, function (data, world_map_country) {
    //   historical_world_map_country_counter.push({
    ...
    // })
    //
    // historical_world_map_country_counter = historical_world_map_country_counter.sort((a, b) => (a.count < b.count) ? 1 : ((b.count < a.count) ? -1 : 0))
    //
    // let _top_historical_world_map_country = (historical_world_map_country_counter.length > TOP) ? TOP : historical_world_map_country_counter.length
    //
    // for (let i = 0; i < _top_historical_world_map_country; i++) {
    //   let data = historical_world_map_country_counter[i]
    //
    //   historical_top_world_map_country_counter.push(data)
    // }
    //
    // let top_city_counter = {}
    // let _top_city_counter = []
    // Object.each(historical_city_counter, function (data, city) {
    ...
    // Object.each(historical_country_counter, function (data, country) {
    ...
    // }
    //
    // debug('HISTORICAL WEB GEOIP',
    //   historical_city_counter,
    //   historical_country_counter,
    //   top_city_counter,
    //   top_country_counter,
    //   historical_continent_counter,
    //   historical_world_map_city_counter,
    //   historical_top_world_map_city_counter,
    //   historical_world_map_country_counter,
    //   historical_top_world_map_country_counter
    // )
    //
    // // vm.stat_world_map_country_counter = historical_world_map_country_counter
    // // vm.stat_world_map_city_counter = historical_world_map_city_counter
    // vm.geodata = {
    //   city_counter: historical_city_counter,
    //   country_counter: historical_country_counter,
    //   top_city_counter: top_city_counter,
    //   top_country_counter: top_country_counter,
    //   continent_counter: historical_continent_counter,
    //   world_map_city_counter: historical_world_map_city_counter,
    //   top_world_map_city_counter: historical_top_world_map_city_counter,
    //   world_map_country_counter: historical_world_map_country_counter,
    //   top_world_map_country_counter: historical_top_world_map_country_counter
    // }
    //
    // vm.traffic = {
    //   status_counter: historical_status_counter,
    //   methods_counter: historical_methods_counter,
    //   // pathnames_counter: historical_pathnames_counter,
    //   top_pathnames_counter: top_pathnames_counter,
    //   type_counter: historical_type_counter,
    //   bytes_counter: historical_bytes_counter,
    //   requests_counter: historical_requests_counter,
    //   // addr_counter: historical_addr_counter,
    //   top_addr_counter: top_addr_counter,
    //   user_counter: historical_user_counter,
    //   referer_counter: historical_referer_counter,
    //   user_agent_os_counter: historical_user_agent_os_counter,
    //   user_agent_engine_counter: historical_user_agent_engine_counter,
    //   user_agent_device_counter: historical_user_agent_device_counter,
    //   user_agent_browser_counter: historical_user_agent_browser_counter,
