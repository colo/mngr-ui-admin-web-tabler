...
<!-- {{config.params.value}} -->
  ...
  <span class="easyPieChartLabel" style="font-size: 20px; top: 63px;">{{format_value(value || config.params.value || percent || config.params.percent )}}</span>
  <span class="easyPieChartUnits" style="font-size: 10px; top: 97px;">{{format_value(max)}} {{params.unit}}</span>

    <vue-easy-pie-chart
      v-bind="Object.merge(params, config.params)"
      :percent="percent || config.params.percent || value || config.params.value "
      :id="id"
      :ref="id"
      v-on:step="step"
    />
...
export default {
  ...
  computed: {
    ...
    graphOptions: function () {
      let options = Object.merge(this.defaultOptions, this.config.params)
  ...
  watch: {
    ...
    dark: function () {
      ...
      this.params = Object.merge(this.graphOptions, this.config.params)
    },
  ...
  methods: {
    ...
    step: function (from, to, percent) {
      // if (this.config.params.max) {
      //   percent = (percent < this.config.params.max) ? (percent * 100 / this.config.params.max) : 100
      //
      ...
      // debug('step', from, to, percent, this.config.params.max, document.getElementById(this.id))
    },
    update (data) {
